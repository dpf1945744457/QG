    ...
    <div class="container_tw">
      ...
      >
        ...
        >
          ...
          <el-form-item>
            <el-button
              v-if="title=='添加BC商品类目及税号'"
              type="warning"
              @click="addquarters('form')"
            >确定</el-button>
            <el-button
              v-if="title=='修改BC商品类目及税号'"
              type="success"
...
export default {
  ...
  methods: {
    ...
    addquarters(formName) {
      this.$refs[formName].validate((valid) => {
        if (valid) {
          ...
          // console.log(this.form, 'sssssssssssss')
          getAdd(this.form)
            .then((response) => {
              if (response.code == '200') {
    ...
    // 上传文件个数超过定义的数量
    action() {
      return 'http://192.168.0.147:9502/bcTax/import' //
    },
